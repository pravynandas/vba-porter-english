...
Public Sub VBAPorter_update()
    ...
    If removeComponent = False Then
        popupMsgQuiet getMsg(FAILED_REMOVE)
        Exit Sub
    End If

    ' Get config
    If Not existConfigFile Then
        popupMsgQuiet formatString(getMsg(NONE_CONFIG), getConfigFolderPath)
        Exit Sub
    End If
    ...
    For i = 0 To UBound(configs)
        ...
        If Not fso.FolderExists(dirpath) Then
            popupMsgQuiet formatString(getMsg(NONE_ROOTPATH), configs(i), dirpath)
        Else
...
Private Function createMenu() As CommandBarPopup
    ...
    menubtn.OnAction = "VBAPorter.VBAPorter_save"
    Set menubtn = childmenu.Controls.Add(Type:=MsoControlType.msoControlButton)
    menubtn.Caption = getMsg(MENU_IMPORT)
    menubtn.OnAction = "VBAPorter.VBAPorter_update"
    ...
    With ThisWorkbook.VBProject
        For Each com In .VBComponents
            If isExportComponent(com.Name, com.Type) Then
                ...
                If expath = "" Then
                    popupMsgQuiet formatString(getMsg(NONE_EXPORTPATH), com.Name)
                Else
                    export = True
...
'If it is Sub, it seems that the next process has been executed before the deletion is completed, so I made it a Function
